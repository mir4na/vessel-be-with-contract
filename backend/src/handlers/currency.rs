...
pub async fn get_supported_currencies(state: web::Data<AppState>) -> AppResult<HttpResponse> {
...
pub async fn get_locked_exchange_rate(
    ...
    let result = state
        .currency_service
        .get_locked_exchange_rate(&body.from_currency, body.amount)
        .await?;
...
pub async fn calculate_estimated_disbursement(
    ...
    let estimate = state
        .currency_service
        .calculate_disbursement_estimate(query.amount);
    Ok(HttpResponse::Ok().json(ApiResponse::success(
        estimate,
        "Disbursement estimate calculated",
    )))
}
