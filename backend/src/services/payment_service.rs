...
use crate::repository::{UserRepository, TransactionRepository, FundingRepository, InvoiceRepository};
...
impl PaymentService {
    ...
    pub async fn get_balance(&self, user_id: Uuid) -> AppResult<UserBalanceResponse> {
        ...
        Ok(UserBalanceResponse {
            balance_idrx: user.balance_idr.to_f64().unwrap_or(0.0),
            currency: "IDRX".to_string(),
        })
    }

    pub async fn deposit(&self, user_id: Uuid, amount: f64, tx_hash: &str) -> AppResult<UserBalanceResponse> {
        // Verify transaction on blockchain
        let is_valid = self.blockchain_service.verify_transaction(tx_hash).await?;
        if !is_valid {
            return Err(AppError::BlockchainError("Transaction not confirmed".to_string()));
        }
        ...
        let new_balance = user.balance_idr + deposit_amount;

        // Update balance
        self.user_repo.update_balance(user_id, new_balance).await?;

        // Record transaction
        self.tx_repo.create_balance_transaction(
            user_id,
            "deposit",
            deposit_amount,
            user.balance_idr,
    ...
    pub async fn withdraw(&self, user_id: Uuid, amount: f64, _to_address: &str) -> AppResult<UserBalanceResponse> {
        let user = self.user_repo.find_by_id(user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        ...
        if user.balance_idr < withdraw_amount {
            return Err(AppError::InsufficientBalance);
        }

        let new_balance = user.balance_idr - withdraw_amount;
    ...
    pub async fn admin_grant_balance(&self, user_id: Uuid, amount: f64, reason: &str) -> AppResult<UserBalanceResponse> {
        let user = self.user_repo.find_by_id(user_id).await?
            .ok_or_else(|| AppError::NotFound("User not found".to_string()))?;
        ...
        let new_balance = user.balance_idr + grant_amount;
