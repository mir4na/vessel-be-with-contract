...
use crate::models::{RegisterRequest, MitraApplication};
use crate::repository::{UserRepository, MitraRepository, OtpRepository};
...
pub async fn setup_services(pool: PgPool) -> Arc<AuthService> {
    ...
    crate::database::run_migrations(&pool).await.expect("Failed to run migrations in test setup");
...
async fn test_register_mitra_success() {
    ...
    let otp_token = jwt_manager.generate_otp_token(email, "registration").expect("Failed to generate OTP token");
    ...
    sqlx::query!("DELETE FROM users WHERE email = $1", email)
        .execute(&pool).await.ok(); // cascading delete should handle mitra_applications if set up, otherwise separate delete needed
    sqlx::query!("DELETE FROM mitra_applications WHERE user_id IN (SELECT id FROM users WHERE email = $1)", email)
        .execute(&pool).await.ok();
    ...
    let req = RegisterRequest {
        ...
        otp_token, // JWT token
        company_name: "Test Mitra Integration PT".to_string(),
        company_type: Some("PT".to_string()),
        npwp: "123456789012345".to_string(),
        annual_revenue: "1000000".to_string(),
        address: None,
...
async fn test_register_fail_invalid_otp() {
    ...
    let req = RegisterRequest {
        ...
        company_name: "Fail PT".to_string(),
        company_type: None,
        npwp: "123".to_string(),
        annual_revenue: "0".to_string(),
        address: None,
        business_description: None,
        website_url: None,
        year_founded: None,
